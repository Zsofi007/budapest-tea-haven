    ...
    <section id="menu" className="py-24 px-6 bg-secondary/20 relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <span className="inline-block text-emerald-300 font-sans text-sm tracking-[0.2em] uppercase mb-4 font-medium">
            {t("menu.sectionLabel")}
          </span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {teas.map((category, index) => (
            <div
              key={index}
              className={`${category.color} p-8 rounded-2xl elevation-2 hover:elevation-4 hover:border-emerald-500/50 border border-border/50 transition-all duration-300 group cursor-pointer relative overflow-hidden`}
              style={{ 
                ...
                background: category.color.includes('brown') || category.color.includes('orange') || category.color.includes('wood') || category.color.includes('black') || category.color.includes('blue')
                  ? `linear-gradient(135deg, hsl(150, 30%, 16%) 0%, hsl(150, 32%, 14%) 100%)`
                  : undefined
              }}
            >
              {/* Premium green/gold hover glow effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/0 via-emerald-500/0 to-emerald-500/0 group-hover:from-emerald-500/12 group-hover:via-emerald-400/6 group-hover:to-transparent transition-all duration-500 rounded-2xl" />
              <div className="absolute inset-0 bg-gradient-to-br from-amber-500/0 via-amber-500/0 to-amber-500/0 group-hover:from-amber-500/6 group-hover:via-amber-400/3 group-hover:to-transparent transition-all duration-500 rounded-2xl" />
              
              <div className="relative z-10">
                <h3 className="font-serif text-2xl text-foreground mb-2 group-hover:text-emerald-200 transition-colors duration-300">
                  {t(category.nameKey)}
                </h3>
                ...
                <ul className="space-y-2">
                  {(t(category.itemsKey) as string[]).map((item, idx) => (
                    ...
                    >
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-300 group-hover:scale-125 transition-transform duration-300" />
                      {item}
                    </li>
                  ))}
                </ul>
